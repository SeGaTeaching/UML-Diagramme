...
![Notationselement Akteur](../grafiken/uc-elem-actor.svg)
...
![Notationselement Anwendungsfall](../grafiken/uc-elem-usecase.svg)
...
![Notationselement Systemgrenze](../grafiken/uc-elem-boundary.svg)
...
![Notationselement Assoziation](../grafiken/uc-elem-assoc.svg)
...
![Notationselement Include-Beziehung](../grafiken/uc-elem-include.svg)
...
![Notationselement Extend-Beziehung](../grafiken/uc-elem-extend.svg)
...
|---------|-------------|-----------|
| **Akteur (Actor)** | ![Akteur](../grafiken/uc-elem-actor.svg) | Rolle oder externes System, das mit dem System interagiert |
| **Anwendungsfall (Use Case)** | ![Anwendungsfall](../grafiken/uc-elem-usecase.svg) | Konkretes Ziel, das ein Akteur mit dem System erreichen will |
| **Systemgrenze** | ![Systemgrenze](../grafiken/uc-elem-boundary.svg) | Rechteck, das zeigt, was zum System gehört und was nicht |
| **Assoziation** | ![Assoziation](../grafiken/uc-elem-assoc.svg) | Durchgezogene Linie: Der Akteur kann diesen Use Case ausführen ("nutzt") |
| **Include** | ![Include](../grafiken/uc-elem-include.svg) | Gestrichelte Linie mit `«include»`: Basis-Use-Case enthält den anderen **immer** als Teilschritt |
| **Extend** | ![Extend](../grafiken/uc-elem-extend.svg) | Gestrichelte Linie mit `«extend»`: Erweiterungs-Use-Case erweitert den Basis-Use-Case **optional** |
...
![Use-Case-Diagramm Geldautomat: Geld abheben, Kontostand abfragen](../grafiken/uc-02.svg)
...
![Use-Case-Diagramm Bibliothekssystem](../grafiken/uc-03.svg)
...
![Akteur-Generalisierung: Premium-Kunde erbt von Kunde](../grafiken/uc-07.svg)
...
![Use-Case-Generalisierung: Bezahlen als Oberfall](../grafiken/uc-08.svg)
...
> **Merksatz:**
> Generalisierung ist die „ist-ein"-Beziehung: durchgezogene Linie, hohles Dreieck, Spitze zum Allgemeineren. Der Spezialfall erbt alles vom allgemeinen Fall.

> **Warnung:**
> Die Generalisierung (durchgezogene Linie, Dreieck) wird leicht mit `«include»`/`«extend»` (gestrichelte Linien, offene Pfeilspitze) verwechselt. Merke: **durchgezogen + Dreieck = Vererbung**, **gestrichelt + offener Pfeil = include/extend**.
...
![Use-Case-Diagramm Include-Beziehung Geldautomat](../grafiken/uc-04.svg)
...
![Use-Case-Diagramm Extend-Beziehung Ticketsystem](../grafiken/uc-05.svg)
...
| Aspekt | Generalisierung | `«include»` | `«extend»` |
|--------|-----------------|-------------|------------|
| **Frage** | ist-ein? | immer nötiger Teilschritt? | optionale Erweiterung? |
| **Notation** | durchgezogene Linie, **hohles Dreieck** | gestrichelte Linie, offener Pfeil, `«include»` | gestrichelte Linie, offener Pfeil, `«extend»` |
...
> **Prüfungstipp (IHK):**
> In der Prüfung wird häufig verlangt, aus einem Anforderungstext ein Use-Case-Diagramm zu erstellen oder ein gegebenes Diagramm zu lesen. Achte besonders auf die **Pfeilrichtung** bei `«extend»` (zeigt auf den Basisfall, also *gegen* den Ablauf) und auf die **Unterscheidung der Linienarten** (durchgezogen+Dreieck = Generalisierung, gestrichelt = include/extend). Ein sehr häufiger Fehler ist, `«include»` und `«extend»` zu vertauschen — merke: **include = immer (Pflicht), extend = manchmal (Kür)**.

> **Warnung:**
> Verwenden Sie Beziehungen sparsam. Zu viele Include-, Extend- und Generalisierungspfeile machen das Diagramm unübersichtlich. Oft reicht ein einfaches Diagramm aus Akteuren, Use Cases und Systemgrenze.
...
![Vom Use-Case-Diagramm zum Code: Geldautomat](../grafiken/uc-06.svg)
...
- **include und extend vertauscht** — Pflichtschritt als `«extend»`, optionale Erweiterung als `«include»`. Folge: falsche Aussage über den Ablauf. Merke: include = immer, extend = manchmal.
- **Falsche Pfeilrichtung bei extend** — Pfeil vom Basisfall zur Erweiterung statt umgekehrt. Folge: Semantik verdreht. Der extend-Pfeil zeigt auf den Basisfall.
